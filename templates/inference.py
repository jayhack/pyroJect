...
class BaseInference(object):
	"""
		...
		------------
			...
			Development should focus on filling in the _pipeline 
			property's getter and setter. _pipeline should support 
	...
	def train(self, data, verbose=False):
		...
		X, y = self.featurize(data, verbose)
...
class ScipyInference(BaseInference):
	...
	def evaluate(self, cv=5):
		...
		scores = cross_val_score(self._pipeline, X, y, cv=5, verbose=2)
